        ...
        <div className="grid grid-cols-8 gap-10">
          ...
          <div className="col-span-6 bg-white">
              ...
              return (
                <div key={product.id}>
                  <div className="grid grid-cols-12 divide-y divide-grey-400 mr-4">
                    <div className="col-span-10 grid grid-cols-8 divide-y divide-grey-400">
                      ...
                      <div className="col-span-6">
                        <div className="font-medium text-black mt-2">
                          <Link to={`/product/${product.id}`}>
                            <ProductDetails product={product} ratings={false}></ProductDetails>
                          </Link>
                        </div>
                        <div>
                          <button className="text-sm xl:text-base font-semibold rounded text-blue-500 mt-2 mb-1" onClick={() => dispatch(removeFromCart(product.id))}>
                            Delete
                          </button>
                        </div>
                        <div className="grid grid-cols-3 w-20 text-center">
                          <div className="text-xl xl:text-2xl bg-gray-400 rounded" onClick={() => dispatch(decrementInCart(product.id))}>
                            -
                          </div>
                          <div className="text-lg xl:text-xl bg-gray-200">{product.quantity}</div>
                          <div className="text-xl xl:text-2xl bg-gray-400 rounded" onClick={() => dispatch(incrementInCart(product.id))}>
                            +
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="col-span-2">
                      <div className="text-lg xl:text-xl mt-2 mr-4">{GB_CURRENCY.format(product.price)}</div>
                    </div>
                  </div>
                </div>
              );
            ...
            <div className="text-lg xl:text-xl text-right mb-4 mr-4">
              Subtotal ({itemsNumber} items): <span className="font-semibold">{GB_CURRENCY.format(subtotal)}</span>
            </div>
          </div>
